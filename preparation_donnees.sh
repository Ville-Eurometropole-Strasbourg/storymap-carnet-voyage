...
docker run -v "$(pwd)/data":/data openmaptiles/planetiler-openmaptiles:latest --force --download --osm-path=strasbourg-a-st-die.osm.pbf --output=strasbourg-a-st-die.mbtiles